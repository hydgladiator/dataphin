---
keyword: [输入组件, ADB for MySQL 3.0]
---

# 操作ADB for MySQL 3.0组件

本文为您介绍如何配置、复制、删除ADB for MySQL 3.0及如何选择数据的发送方式。

**ADB for MySQL 3.0**输入组件适用于从**ADB for MySQL 3.0**数据库中读取数据至Dataphin平台，进行数据的整合和再加工的场景。
...
6.  将**ADB for MySQL 3.0**组件拖到左侧的管道画布中。

7.  将鼠标放置在**ADB for MySQL 3.0**组件框内右键单击，选择**属性配置**。

8.  根据**ADB for MySQL 3.0输入配置**对话框提示，配置参数。

    ![dFA](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/8093819951/p86849.png)
...
同时您可以单击**数据源**后的![dfag](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/2193819951/p80969.png)图标，进入规划模块添加数据源，详情请参见[新建AnalyticDB for MySQL数据源](/cn.zh-CN/数仓规划/数据源/新建离线数据源/新建AnalyticDB for MySQL数据源.md)。|
    |**来源表量**|选择来源表量。来源表量包括**单表**和**多表**。 **说明：** **多表**指多张具有相同表结构的表。 |
    |**表**|选择来源表。根据选择**来源表量**的类型，选择来源表：     -   如果**来源表量**选择了**单表**，则单击![geg](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/0193819951/p80970.png)图标，选择输入的**表**。
...
9.  单击**确认**，完成**ADB for MySQL 3.0**输入组件的属性配置。


## 复制ADB for MySQL 3.0

1.  将鼠标放置在**ADB for MySQL 3.0**组件框内右键单击，单击**复制**。

2.  在管道画布中右键单击后，选择**粘贴**，完成**ADB for MySQL 3.0**输入组件的复制。


## 删除ADB for MySQL 3.0

1.  将鼠标放置在**ADB for MySQL 3.0**组件框内右键单击，单击**删除**。

2.  在确认弹框中单击**确定**，完成**ADB for MySQL 3.0**输入组件的删除。
...
1.  将鼠标放置在**ADB for MySQL 3.0**组件框内右键单击，单击**数据发送方式**。
