---
keyword: [输入组件, SQL Server]
---

# 管理SQL Server组件

本文为您介绍如何配置、复制、删除SQL Server组件及如何选择数据的发送方式。

**SQL Server**输入组件适用于从**SQL Server**数据库中读取数据至Dataphin平台，进行数据的整合和再加工的场景。
...
6.  将**SQL Server**组件拖到左侧的管道画布中。

7.  鼠标悬停至**SQL Server**组件框内右键单击，选择**属性配置**。

8.  在**SQL Server输入配置**对话框中，配置参数。

    ![vbza](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/2983819951/p86677.png)

    |参数|描述|
    |--|--|
    |**步骤名称**|根据当前组件的使用场景，填写名称。|
    |**数据源**|选择数据源。选取Dataphin系统中已配置的数据源，且数据源需同时具备以下两个条件：     -   数据源类型为SQL Server。
    -   执行**属性配置**的账号具有该数据源的同步读权限，如果没有权限，则需要申请数据源权限，详情请参见[管理数据源权限](/cn.zh-CN/资产中心/安全中心/我的权限/操作数据源权限.md)。
同时您可以单击**数据源**后的![dfag](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/2193819951/p80969.png)图标，进入规划模块添加数据源，详细内容请参见[新建SQL Server数据源](/cn.zh-CN/数仓规划/数据源/新建离线数据源/新建SQL Server数据源.md)。|
...
9.  单击**确认**，完成**SQL Server**输入组件的属性配置。


## 复制SQL Server组件

1.  鼠标悬停至**SQL Server**组件框内右键单击，单击**复制**。

2.  在管道画布中右键单击后，选择**粘贴**，完成**SQL Server**输入组件的复制。


## 删除SQL Server组件

1.  鼠标悬停至**SQL Server**组件框内右键单击，单击**删除**。

2.  在确认弹框中单击**确定**，完成**SQL Server**输入组件的删除。
...
1.  鼠标悬停至**SQL Server**组件框内右键单击，单击**数据发送方式**。
